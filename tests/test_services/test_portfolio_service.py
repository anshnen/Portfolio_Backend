...
from app.models.models import User, Portfolio, Account, Asset, Holding, Transaction
...
def test_get_portfolio_summary_logic(db):
    ...
    cash = Account(name="Cash", account_type="CASH", balance=Decimal("10000"), portfolio=portfolio)
    brokerage = Account(name="Brokerage", account_type="INVESTMENT", portfolio=portfolio)
    ...
    t1 = Transaction(account=cash, transaction_type="DEPOSIT", total_amount=5000, transaction_date=date.today())
    t2 = Transaction(account=cash, transaction_type="WITHDRAWAL", total_amount=-2000, transaction_date=date.today())
